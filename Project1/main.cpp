...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        ...
        case WM_PAINT:
        {
            ...
            for (int y = 0; y < H * ratio; y++)
            {
                for (int x = 0; x < W * ratio; x++)
                {
                    ...
                    Color col = frame.GetPixel(x / ratio, y / ratio);
                    pixel->RED = col.red;
                    pixel->GREEN = col.green;
                    pixel->BLUE = col.blue;
                    pixel->ALPHA = col.alpha;
                }
            }
        ...
        case WM_MOUSEMOVE:
        {
            ...
            sprintf_s(str, "X = %d, Y = %d, BX = %d, BY = %d", LOWORD(lParam), HIWORD(lParam), LOWORD(lParam) / pixelSize, HIWORD(lParam) / pixelSize);
        ...
        case WM_KEYDOWN:
        {
            ...
            if (wParam == VK_F1)
            {
                MessageBoxA(hWnd, "Работу списала студент группы ПВ-231 Чупахина С.А.", "О программе", MB_ICONINFORMATION);
            }
